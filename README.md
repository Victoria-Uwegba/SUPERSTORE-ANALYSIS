# SUPERSTORE-ANALYSIS


![Dashboard Preview](./superstore%202014%20analysis.PNG)

## Overview
Comprehensive retail analysis platform for tracking Superstore's 2014 performance across key dimensions:
- Regional sales distribution
- Product category performance
- Weekly sales patterns
- Seasonal shopping trends

## Key Metrics
- **Total Revenue**: \[$136,583\]
- **Units Sold**: \[595\] items
- **Average Discount Rate**: \[3.4%\]
- **Peak Sales Month**: August (\[$25,061.30\])

## Core Insights
### 1. Regional Performance
| Region | Sales Contribution |
|--------|--------------------|
| West   | 32.67%            |
| East   | 26.11%            |
| South  | 22.36%            |
| Central| 18.86%            |

### 2. Product Category Breakdown
- **Technology**: \[$21,839.19\] total sales
- **Office Supplies**: \[$12,438.29\]
- **Furniture**: \[$9,365.44\]

## Strategic Recommendations
### Inventory Optimization
- **Top Performers**:
  - Paper (\[$30,199.33\])
  - Tables (\[$27,296.64\])
  - Accessories (\[$17,931.86\])
- **Action**: Increase stock levels by 25% for the top 3 sub-categories

### Regional Focus
- Prioritize West region fulfillment centers
- Develop targeted promotions for the Central region (lowest contribution)

- Capitalize on holiday tech sales (November/December peak)
- Bundle office supplies with furniture during back-to-school season

